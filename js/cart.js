...
function addToCart(shopId, product, btn) {
  ...
  const existing = cart.find(
    item => item.id === product._id && item.shopId === shopId
  );
  ...
  image: product.image.startsWith("http")
    ? product.image
    : `https://mieza.onrender.co${product.image}`,
...
function renderCartItems() {
  ...
  <button
    onclick="changeQty('${item.shopId}','${item.id}', -1)"
  >
    −
  </button>
  ...
  <button
    class="remove-btn"
    onclick="removeItem('${item.shopId}','${item.id}')"
  >
    Remove
  </button>
